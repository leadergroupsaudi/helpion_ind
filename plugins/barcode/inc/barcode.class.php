...
class PluginBarcodeBarcode {
      ...
      echo "<form name='form' method='post'
        ...
        echo "<table class='tab_cadre'>";
         ...
         echo "<tr><td class='tab_bg_1' colspan='4' align='center'>
                   <input type='submit' value='".__('Print', 'barcode')."'
                          class='submit'></td></tr>";
        echo "</table>";
        echo "</div>";
        Html::closeForm();
   ...
   function showFormMassiveAction(MassiveAction $ma) {
      ...
      if (key($ma->items) == 'Ticket') {
        ...
        // Add a professional note about the inventory number format
        echo '<br/><small style="color:red;">';
        echo __('Please note: The Inventory number must be alphanumeric and should not include any special characters, such as "/".', 'barcode');
        echo '</small>';
   ...
   static function commonShowMassiveAction() {
      ...
      echo "<br/>";
      echo Html::submit(__('Print', 'barcode'), ['value' => 'create']);
   }
      ...
      switch ($ma->getAction()) {
         ...
         case 'Generate' :
            ...
            if (count($codes) > 0) {
               ...
               $msg = "<a target='_blank' href='".Plugin::getWebDir('barcode').'/front/send.php?file='.urlencode($filename)
