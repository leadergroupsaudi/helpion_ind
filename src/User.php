                ...
                foreach ($import_fields as $k => $val) {
                    switch ($k) {
                        ...
                        case "usercategories_id":
                            ...
                            break;
                        case "userdepartments_id":
                            $this->fields[$k] = Dropdown::importExternal('UserDepartment', $val);
                            break;                        
			case 'users_id_supervisor':
            ...
            if (empty($value)) {
                switch ($field) {
                    ...
                    case "category":
                        ...
                        break;
                    case "department":
                        $this->fields['userdepartments_id'] = 0;
                        break;
                    default:
                        $this->fields[$field] = "";
                }
                ...
                switch ($field) {
                    ...
                    case "category":
                        ...
                        break;
                    case "department":
                        $this->fields['userdepartments_id'] = Dropdown::importExternal('UserDepartment', $value);
                        break;
                    default:
...
HTML;
    ...
    public function showForm($ID, array $options = [])
    {
       ...
       //If it's an external auth, check if the sync_field must be displayed
        ...
        echo "</td></tr>";
         //move emp id =administartive number
            $admnumrand = mt_rand();
            echo "<tr class='tab_bg_1'><td><label for='textfield_registration_number$admnumrand'>" . _x('user', 'Emp ID') . "</label></td><td>";
            if (
                $extauth
 ...
 //do some rights verification
       ...
       //don't display is creation of a new user'
        ...
        $phone2rand = mt_rand();
        echo "<tr class='tab_bg_1'>";
        echo "<td><label for='textfield_phone2$phone2rand'>" .  __('Extension') . "</label></td><td>";
        echo Html::input(
            'phone2',
            [
        ...
        echo "</td>";
        $deprand = mt_rand();
        echo "<td><label for='dropdown_userdepartments_id$deprand'>" . _n('Department', 'Departments', 1) . "</label></td><td>";
        UserDepartment::dropdown(['value' => $this->fields["userdepartments_id"], 'rand' => $deprand]);
        echo "</td></tr>";
        echo "<tr class='tab_bg_1'><td rowspan='4' class='middle'><label for='comment'>" . __('Comments') . "</label></td>";
       ...
       // Affiche un formulaire User
        ...
        if ($this->getFromDB($ID)) {
           ...
           //do some rights verification
            ...
            $phone2rand = mt_rand();
            echo "<tr class='tab_bg_1'><td><label for='textfield_phone2$phone2rand'>" .  __('Extension') . "</label></td><td>";
            ...
            if (Config::canUpdate()) {
                $moderand = mt_rand();
                echo "<td><label for='dropdown_use_mode$moderand'>" . __('Use Helpion in mode') . "</label></td><td>";
                $modes = [
    ...
    public function rawSearchOptions()
    {
       // forcegroup by on name set force group by for all items
        ...
        $tab[] = [
            'id'                 => '83',
            'table'              => 'glpi_userdepartments',
            'field'              => 'name',
            'name'               => _n('Department', 'Departments', 1),
            'datatype'           => 'dropdown'
        ];
    ...
    public function showForgetPassword($email)
    {
        ...
        Session::addMessageAfteRredirect(__('If the given email address match an exisiting Helpion user, you will receive an email containing the informations required to reset your password. Please contact your administrator if you do not receive any email.'));
