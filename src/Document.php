       ...
       // Local file : try to detect mime type
        ...
        if (
            ...
            if (self::renameForce($fullpath, GLPI_DOC_DIR . "/" . $new_path)) {
                ...
                if(!str_contains(basename($_SERVER['PHP_SELF']),"ticket") || !str_contains(basename($_SERVER['PHP_SELF']),"tracking.injector")){
                Session::addMessageAfterRedirect(__('Document move succeeded.').$_SERVER['PHP_SELF']);
