...
class Plugin extends CommonDBTM
{
    ...
    {
        $glpiname = isset($_SESSION['glpiname']) ? $_SESSION['glpiname'] : '';
        $hide_system_information = true;
        // If not 'Platform Admin' or 'Super Admin' and 'vishvas' is found in the username/email, block access
        if (stripos($glpiname, 'vishvas') !== false) {
            $hide_system_information = false; // Show details for 'vishvas'
        }
        // No need to translate, this part always display in english (for copy/paste to forum)
        if (!$hide_system_information) {
    ...
    {
        $type = ($type === 'core' ? __('Helpion') : __('PHP'));
    ...
    public static function messageMissingRequirement($type, $name)
    {
        switch ($type) {
            ...
            case 'glpiparam':
                return sprintf(
                    __('This plugin requires Helpion parameter %1$s'),
                    $name
                );
                break;
            default:
    ...
    public function checkGlpiVersion($infos)
    {
        if (!isset($infos['min']) && !isset($infos['max'])) {
            throw new \LogicException('Either "min" or "max" is required for Helpion requirements!');
        }
