                ...
                if ($link == 2) {
                    ...
                    if ($anon_name === null) {
                        $user_params = array_merge($user_params, [
                            ...
                            'userdepartments_id'  => $data['userdepartments_id'],
    ...
    public function getDbRelations()
    {
       ...
       // Add plugins relations
        ...
        foreach ($RELATION as $source_table => $table_relations) {
            ...
            if($source_table=="glpi_userdepartments"){
                $source_itemtype="UserDepartment";}
            if (!is_a($source_itemtype, CommonDBTM::class, true)) {
                trigger_error(
                    sprintf(
                        'Invalid relations declared for "%s" "%s" table. Table does not correspond to a known itemtype.',
                        $source_table,$source_itemtype
                    ),
                    E_USER_WARNING
                );
                continue;
