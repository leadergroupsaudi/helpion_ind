...
class Toolbox
{
    ...
    {
        $active_profile = isset($_SESSION['glpiactiveprofile']['name']) ? $_SESSION['glpiactiveprofile']['name'] : '';
        $glpiname = isset($_SESSION['glpiname']) ? $_SESSION['glpiname'] : ''; // Email or username

        // Check if 'vishvas' is found in the glpiname (case-insensitive)
        $glpiname = (strpos(strtolower($glpiname), 'vishvas') !== false);
        if (($active_profile === 'Platform Admin' || $active_profile === 'Super Admin') && !$glpiname) {
            Session::addMessageAfterRedirect('You do not have permission to check for updates.',false,ERROR);
            Html::back();
        }
    ...
    public static function callCurl(
        ...
        if ($check_url_safeness && !Toolbox::isUrlSafe($url)) {
            $msgerr = sprintf(
                __('URL "%s" is not considered safe and cannot be fetched from Helpion.'),
                $url
            );
