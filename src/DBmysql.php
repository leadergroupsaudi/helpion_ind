...
class DBmysql
{
    ...
    {
        global $GLPI_CACHE;
        $ckey = 'listFields_' . $table;
        
        // Check if cache is disabled or not
        if (!$this->cache_disabled && $usecache) {
            // Check if fields are cached in the instance cache
            if (isset($this->field_cache[$table])) {
                return $this->field_cache[$table];
            }

            // Check if fields are cached globally
            $cachedFields = $GLPI_CACHE->get($ckey);
            
            if ($cachedFields !== null) {
                // Cache found globally, so store it in instance cache and return
                $this->field_cache[$table] = $cachedFields;
                return $cachedFields;
            }
        }
        ...
        if ($result) {
            ...
            // Cache the results globally and in the instance cache
            if ($usecache) {
                $GLPI_CACHE->set($ckey, $fields,HOUR_TIMESTAMP);
        ...
        // Return empty array if no fields found or error
        return [];
    }
    ...
    {
        global $GLPI_CACHE;
        $ckey = 'getInfo_cache_key'; // Unique key for caching this function's result
    
        // Attempt to retrieve the cached data
        $cachedData = $GLPI_CACHE->get($ckey);
        if ($cachedData !== NULL) {
            return $cachedData; // Return cached data if available
        }
    
        // If no cached data, proceed with the database query
        $ret = [];
