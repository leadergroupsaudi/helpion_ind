...
class View extends CommonGLPI
{
    ...
    public static function checkRegistrationStatus()
    {
        ...
        if (!GLPINetwork::isServicesAvailable()) {
            array_push(
                $messages,
                sprintf(__('%1$s services website seems not available from your network or offline'), 'Helpion Network'),
            ...
            if (!$registration_info['is_valid']) {
                ...
                array_push(
                    $messages,
                    sprintf(__('Your %1$s registration is not valid.'), 'Helpion Network'),
                    __('A registration, at least a free one, is required to use marketplace!'),
                    "<a href='" . GLPI_NETWORK_SERVICES . "'>" . sprintf(__('Register on %1$s'), 'Helpion Network') . "</a> "
                        . __('and') . " "
                ...
                array_push(
                    $messages,
                    sprintf(__('Your %1$s subscription has been terminated.'), 'Helpion Network'),
                    "<a href='" . GLPI_NETWORK_SERVICES . "'>" . sprintf(__('Renew it on %1$s.'), 'Helpion Network') . "</a> "
                );
            } else {
                $valid = true;
            }
    ...
    public static function displayList(
        ...
        if ($is_list_truncated) {
            $msg = count($plugins) === 0
                ? sprintf(__('Unable to fetch plugin list due to %s services website unavailability. Please try again later.'), 'Helpion Network')
                : sprintf(__('Plugin list may be truncated due to %s services website unavailability. Please try again later.'), 'Helpion Network');
...
HTML;
    ...
    /**
     * Return HTML part for Helpion Network information for a given plugin
     * @param array $plugin data of the plugin.
        ...
        if (count($require_offers)) {
            ...
            $html = "<div class='offers'>
                    <a href='" . GLPI_NETWORK_SERVICES . "' target='_blank'
                       ...
                       title='" . sprintf(__s("You must have a %s subscription to get this plugin"), 'Helpion Network') . "'>
                        <i class='fas fa-star'></i>Helpion Network
                    </a>
                    <a href='" . GLPI_NETWORK_SERVICES . "' target='_blank'
